			...
			$(function(){
				...
				$(".inNum").change(function(){
					...
					if(numReg.test(num)){
						var url="<%=path%>/cart/modify";
						$.post(url,
						{"cartId":cartId,"edit":2,"cartNum":num},
						function(res){
							if(res.code==200){
								numObj.val(num);
                                countTotal();
			...
			<table border="1" cellpadding="0" cellspacing="0" class="table table-bordered">
				...
				<tbody>
					...
					<tr>
						...
						<td><img src="<%=path%>/resources/upload/${cart.goodsimg}"/><span>${cart.goodsname}</span></td>
						<td><span class="price">${cart.price}</span></td>
						<td><input type="button" value="-" class="subNum"/>
							<input type="text" value="${cart.num}"  class="inNum"/>
							<input type="button" value="+" class="addNum"/>
							<input type="hidden" value="${cart.product}"/></td>
